...
public unsafe partial struct Image : IDisposable
{
	...
	[DllImport("raylib.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "LoadImage")]
	public static extern Image Load(string fileName);

	[DllImport("raylib.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "LoadImageRaw")]
	public static extern Image LoadRaw(string fileName, int width, int height, PixelFormat pixelFormat, int headerSize);

	[DllImport("raylib.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "LoadImageFromTexture")]
	public static extern Image Load(Texture texture);

	[DllImport("raylib.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "IsImageValid")]
	[return: MarshalAs(UnmanagedType.I1)]
	private static extern bool IsThisValid(Image image);
	public bool IsValid => IsThisValid(this);

	[DllImport("raylib.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "UnloadImage")]
	private static extern void Unload(Image image);

	[DllImport("raylib.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "ExportImage")]
	[return: MarshalAs(UnmanagedType.I1)]
